...
class FastSpeechEncoder(nn.Module):
    def __init__(self, vocab_size, hidden_size, num_heads, kernel_size, n_fft_blocks):
        ...
        self.fft_blocks = [FFTBlock(hidden_size, num_heads, kernel_size) for _ in range(n_fft_blocks)]
...
class FastSpeechDecoder(nn.Module):
    def __init__(self, hidden_size, num_heads, kernel_size, n_fft_blocks):
...
class FastSpeech(nn.Module):
    ...
    def forward(self, input, alignes=None):
        ...
        out = []
        if alignes is None:
            ones = torch.ones(length_pred.shape)
            alignes = length_pred
            alignes, _ = torch.max(torch.stack([ones, alignes], dim=0), dim=0)
            alignes = alignes.type(torch.LongTensor)
